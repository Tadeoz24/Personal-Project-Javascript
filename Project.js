...
class Transaction {
  ...
  async dispatch(scenario) {
    ...
    function GenerateSucObject(element, err) {
      var objectRes = {
        ...{},
        ...{
          index: element.index,
    ...
    function GenerateSucObject(storebefore, element, result) {
      var objectRes = {
        ...{},
        ...{
          index: element.index,
    ...
    for (var element of this.scenarioObject) {
      try {
        ...
        this.logs.push(GenerateSucObject(this.store2, element, result));
      } catch (err) {
        try {
          if (typeof element.restore !== "undefined") {
            ...
            this.logs.push(GenerateSucObject(element, err));
            this.store = {};
          }
        } catch (err) {
          this.logs.push(GenerateSucObject(element, err));
